                  ...
                  <li v-for="sct in ct.inserted" :key="sct.id" class="inline-block mr-3 my-1 group">
                    ...
                    <div class="absolute invisible group-hover:opacity-100 group-hover:visible" v-if="sct.inserted.length > 0">
                      ...
                      <div class="bg-white border-gray-200 border rounded-sm dark:border-gray-700 dark:bg-gray-800 p-4">
                        <nuxt-link :to="{ name: 'prods', query: {ct: sct.id, page: 1}}" class="font-bold">{{ sct.name }}</nuxt-link>
                        <div class="my-2 md:columns-2">
                          <div v-for="ssct in sct.inserted" :key="ssct.id" class="px-1">
                            <nuxt-link :to="{ name: 'prods', query: {ct: ssct.id, page: 1}}" class="text-sm text-gray-700 hover:text-gray-900 dark:text-gray-300 dark:hover:text-gray-100">{{ ssct.name }}</nuxt-link>
                          </div>                                 
                        </div>
                      </div>
